...
# Dictionary with the access_token or ID, and timestamp value for every user's session in the system
user_sessions = {}
...
# Return a newly created user's session, and store it (overwriting) in the dictionary user's entry, along with the tiemstamp value (overwriting).
def _crearSession(_ID):
    ...
    try:
        ...
        user_sessions[_ID] = {"session": session, "timestamp": datetime.datetime.now().timestamp()}
        return session
    except:
...
def _actualizarTimestamp(_ID):
    if (_recuperarTimestamp(_ID) != None):
        user_sessions.get(_ID).update({"timestamp": datetime.datetime.now().timestamp()})
...
def _crearTokenFlag(_ID, _flag):
    ...
    if (user != None):
        ...
        if (Settings.DEBUG): print('DEBUG-cTF-001: User does not exist for the ID: {}'.format(_ID))
        return False
...
def sessionExists(UUID):
    try:
        assert(_recuperarSession(UUID) != None)
        return True
    except:
...
def sessionValid(UUID):
    try:
        ...
        assert((timeStamp != None) and ((timeStamp + SESSION_THRESHOLD >= datetime.datetime.now().timestamp())))
        return True
    except:
        print('Except session Valid')
        return False

    # return True if ((timeStamp != None) and ((timeStamp + SESSION_THRESHOLD < datetime.datetime.now().timestamp()))) else False 
...
def sessionControl(_ID):
    ...
    if (Settings.DEBUG): print('sC INI')

    # Create a new session
    if (_recuperarSession(_ID) == None):
    ...
    # Update an outdated session
        ...
        if (Settings.DEBUG): print('DEBUG-sC-001: Timestamp outdated')
        return _crearSession(_ID)
...
def tokenControl(request, UUID):
    ...
    if (Settings.DEBUG): 
        ...
        try:
            print('DEBUG: UUID: {} - sessionId: {}'.format(UUID,_recuperarSession(UUID).sessionID))
        except:
    ...
    if request.POST.get('msToken', None):
        ...
        try:
            ...
            assert(result.get('sessionId') == _recuperarSession(UUID).sessionID)
            
            try:
 
                result_data = json.loads(result.get('data').replace('null',"\"null\""))
	        ...
	        # Mockup of the result sessionMngrResponse object
            	if (Settings.DEBUG): print('DEBUG: Mocking the callback object within msToken')

            	# *********************
            	result = {'data': {'code': 'OK', 'additionalData': 'msToken received, but no data field found. MOCKED!' }}
            	# *********************        
            	assert(_crearTokenFlag(UUID, result))
        ...
        try:
            # Mockup of the result sessionMngrResponse object
            if (Settings.DEBUG): print('DEBUG: Mocking the callback object without msToken!')

            # *********************
            result = {'data': {'code': 'OK', 'additionalData': 'msToken not received. MOCKED!' }}
            # *********************        
            assert(_crearTokenFlag(UUID, result))
...
def tokenFlag(request, UUID):
    ...
    # The custom callback URL has been resolved
        ...
        try:
            # Please DO NOT DELETE (Future implementation of the SessionMngrResponse object flag validation)
            ...
            # An exception is raised due to an undefined status of the custom callback URL result
            else:
                if (Settings.DEBUG): print('ERROR-tF-002: The undefined code is: ', code)
                assert(_resetearTokenFlag(UUID))
                raise Exception
